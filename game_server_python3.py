...
# The client frame shows - client area
...
# Function to receive message from the current client AND
...
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    update_client_names_display(clients_names)  # update the client names display
    ...
    while True:
        ...
        # Get the player's choice from the received data
        ...
        if len(player_data) == 2:
            # Send player 1's choice to player 2 and vice versa
    ...
    # Find the client's index then remove them from both lists (client name list and connection list)
...
# Update the client name display when a new client connects OR
